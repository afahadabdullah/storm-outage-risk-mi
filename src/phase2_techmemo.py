...
@media (max-width:680px) { main { width:min(100% - 20px, 1040px); } header { padding-top:36px; } section { padding:20px 16px; } .split,.figure-grid { grid-template-columns:1fr; } }
</style>
</head>
<body>
<main>
...
const comma = value => Number.isFinite(Number(value)) ? Math.round(Number(value)).toLocaleString() : "—";
...
function valueChart() {
  ...
  const values=data.cost_loss.filter(row => row.lead_days === 0 && Number.isFinite(Number(row.relative_economic_value))).sort((a,b) => a.cost_loss_ratio-b.cost_loss_ratio);
